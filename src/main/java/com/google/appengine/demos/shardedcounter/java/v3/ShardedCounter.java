...
package com.google.appengine.demos.shardedcounter.java.v3;
...
public class ShardedCounter {
    ...
    public ShardedCounter(final String name) {
        ...
        ancestor = KeyFactory.createKey(kind, "$$");
    }
    ...
    public final void addShards(final int count) {
        Key counterKey = KeyFactory.createKey(ancestor, Counter.KIND, counterName);
        incrementPropertyTx(counterKey, Counter.SHARD_COUNT, count,
                INITIAL_SHARDS + count, false);
    }
...
//        }
        ...
        if (value != null) {
            if (value.longValue() != sum)
            	LOG.warning("memcache = " + value + ", whereas ds = " + sum);
        }
        return sum;
    }
    ...
    public final long increment() {
        ...
        Key shardKey = KeyFactory.createKey(ancestor, kind, Long.toString(shardNum));
        mc.increment(kind, 1);
        return incrementPropertyTx(shardKey, CounterShard.COUNT, 1, 1, true);
    }
     ...
     */
    private long incrementPropertyTx(final Key key, final String prop,
            final long increment, final long initialValue, boolean returnCount) {
        ...
        try {
            ...
            DS.put(thing);
            long count = returnCount ? getCount() : -1;
            tx.commit();
            return count;
